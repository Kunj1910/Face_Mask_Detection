...
while True:
	...
	for x,y,w,h in faces:
		...
		# classsIndex=model.predict_classes(img) 
		classIndex=np.argmax(model.predict(img), axis=-1)
		print(f"Class Index: {classIndex}")
		probabilityValue=np.amax(prediction)
		# print(f"Probability: {probabilityValue}")
		if probabilityValue>threshold:
			if classIndex==0:
				cv2.rectangle(imgOrignal,(x,y),(x+w,y+h),(0,255,0),2)
				cv2.rectangle(imgOrignal, (x,y-40),(x+w, y), (0,255,0),-2)
				cv2.putText(imgOrignal, str(get_className(classIndex)) + f' {(probabilityValue * 100):.2f}%',(x,y-10), font, 0.75, (255,255,255),1, cv2.LINE_AA)
			elif classIndex==1:
				cv2.rectangle(imgOrignal,(x,y),(x+w,y+h),(50,50,255),2)
				cv2.rectangle(imgOrignal, (x,y-40),(x+w, y), (50,50,255),-2)
				cv2.putText(imgOrignal, str(get_className(classIndex)) + f' {(probabilityValue * 100):.2f}%',(x,y-10), font, 0.75, (255,255,255),1, cv2.LINE_AA)
